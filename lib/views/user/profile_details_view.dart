                              ...
                              action: () async {
                                if(Constants.checkUserLoginStatus()){
                                  if(userSnap.data!.get("subscriptionCount") > 0){
                                    ...
                                    var connDoc = await FirebaseFirestore.instance.collection('CareTakers').doc(careTaker.id).collection('Connections').doc(FirebaseAuth.instance.currentUser!.uid).get();
                                    var userConnDoc = await FirebaseFirestore.instance.collection('Users').doc(FirebaseAuth.instance.currentUser!.uid).collection('Connections').doc(careTaker.id).get();
                                    ...
                                    if(!connDoc.exists){
                                      FirebaseFirestore.instance.collection('CareTakers').doc(careTaker.id).collection('Notifications').doc().set(
                                          {
                                      ...
                                      FirebaseFirestore.instance.collection('CareTakers').doc(careTaker.id).collection('Connections').doc(FirebaseAuth.instance.currentUser!.uid).set(
                                          {
                                            "date": DateFormat('dd-MM-yyyy').format(DateTime.now()),
                                            "time": DateFormat('hh:mm a').format(DateTime.now()),
                                            "timestamp": DateTime.now().millisecondsSinceEpoch,
                                    ...
                                    if(!userConnDoc.exists){
                                      FirebaseFirestore.instance.collection('Users').doc(FirebaseAuth.instance.currentUser!.uid).collection('Connections').doc(careTaker.id).set(
                                          {
                                            "date": DateFormat('dd-MM-yyyy').format(DateTime.now()),
                                            "time": DateFormat('hh:mm a').format(DateTime.now()),
                                            "timestamp": DateTime.now().millisecondsSinceEpoch,
